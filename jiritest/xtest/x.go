...
func NewX(t *testing.T) *jiri.X {
	...
	root := t.TempDir()
	// TODO(https://fxbug.dev/356134056): Don't chdir by default, so most tests
	// can run in parallel.
	Chdir(t, root)
...
// Chdir changes the process's current working directory and switches back to
// the original at the end of the test.
