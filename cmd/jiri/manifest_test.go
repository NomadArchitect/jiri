...
func TestReadManifest(t *testing.T) {
	// Override the callback to read a manifest file with our own.  This allows
	// us to avoid placing a manifest file in the source-tree just to run this
	// test, and prevents changes to that manifest from breaking these tests.
...
`))
	...
	runCommand := func(t *testing.T, args []string) (stdout string, stderr string) {
		...
		// Create a new ManifestCommand with our test callback.
		cmd := &ManifestCommand{readManifestFunc: testReadManifest}
