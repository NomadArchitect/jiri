...
func init() {
	flags := &cmdEdit.Flags
	flags.Var(&editFlags.projects, "project", "List of projects to update. It is of form <project-name>=<project-revision> where project-revision is optional. It can be specified multiple times.")
	flags.Var(&editFlags.imports, "import", "List of imports to update. It is of form <import-name>=<import-revision> where import-revision is optional. It can be specified multiple times.")
	flags.Var(&editFlags.packages, "package", "List of packages to update. It is of form <package-name>=<package-version>. It can be specified multiple times.")
...
func runEdit(jirix *jiri.X, args []string) error {
	...
	for _, p := range editFlags.packages {
		s := strings.SplitN(p, "=", 2)
		if len(s) == 1 {
			return jirix.UsageErrorf("Please provide the -package flag in the form <package-name>=<package-version>")
		} else {
			packages[s[0]] = s[1]
		}
	}
