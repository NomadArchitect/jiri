...
func updateManifest(jirix *jiri.X, manifestPath string, projects, imports, packages map[string]string) error {
	...
	for _, p := range m.Projects {
		...
		editedProjects[p.Key()] = newRevision
	...
	if len(editedProjects) != 0 && jirix.LockfileEnabled && (editFlags.editMode == lockfile || editFlags.editMode == both) {
