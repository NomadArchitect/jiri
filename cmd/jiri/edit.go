...
func init() {
	flags := &cmdEdit.Flags
	flags.StringVar(&editFlags.manifest, "manifest", "", "Path of the manifest file w.r.t jiri root")
	flags.Var(&editFlags.projects, "projects", "List of projects to update")
}

func runEdit(jirix *jiri.X, args []string) error {
	if editFlags.manifest == "" {
		jirix.UsageErrorf("Please pass -manifest flag")
	}
	manifestPath := filepath.Join(jirix.Root, editFlags.manifest)
