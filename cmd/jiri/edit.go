...
func init() {
	...
	flags.StringVar(&editFlags.editMode, "edit-mode", "both", "Edit mode. It can be 'manifest', 'lockfile' or 'both'")
}
...
func runEdit(jirix *jiri.X, args []string) error {
	...
	switch strings.ToLower(editFlags.editMode) {
	case "manifest":
		editFlags.editModeI = Manifest
	case "lockfile":
		editFlags.editModeI = Lockfile
	case "both":
		editFlags.editModeI = Both
	default:
...
func updateLockfiles(jirix *jiri.X, manifestPath string, projects map[string]string) (cleanup func(), rewind func(), err error) {
	...
	rewind = func() {
		for k, v := range backup {
			if err := os.Rename(v, k); err != nil {
				jirix.Logger.Errorf("revert changes to lockfile %q failed", k)
			} else {
	...
	dir := manifestPath
	for ; !(dir == "" || dir == "." || dir == jirix.Root || dir == string(filepath.Separator)); dir = path.Dir(dir) {
...
func updateLocks(jirix *jiri.X, tempDir, lockfile string, backup, projects map[string]string) error {
	...
	for k, v := range projectLocks {
		if newRev, ok := projects[v.Name]; ok {
			v.Revision = newRev
			projectLocks[k] = v
			found = true
		}
...
func updateManifest(jirix *jiri.X, manifestPath string, projects, imports, packages map[string]string) error {
	...
	for _, p := range m.Projects {
		...
		if editFlags.editModeI == Manifest || editFlags.editModeI == Both {
	...
	for _, ecp := range ec.Projects {
		editedProjects[ecp.Name] = ecp.NewRev
	}
