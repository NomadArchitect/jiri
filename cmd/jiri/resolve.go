...
var resolveConfig project.ResolveConfig
...
func init() {
	...
	flags.StringVar(&resolveConfig.LockFilePath, "output", "jiri.lock", "Path to the generated lockfile")
	flags.BoolVar(&resolveConfig.LocalManifest, "local-manifest", false, "Use local manifest")
	flags.BoolVar(&resolveConfig.EnablePackageLocks, "enable-package-lock", true, "Enable resolving packages in lockfile")
	flags.BoolVar(&resolveConfig.EnableProjectLocks, "enable-project-lock", false, "Enable resolving projects in lockfile")
	flags.BoolVar(&resolveConfig.EnablePackageVersion, "enable-package-version", false, "Enable version tag for packages in lockfile")
}
...
func runResolve(jirix *jiri.X, args []string) error {
	...
	return project.GenerateJiriLockFile(jirix, manifestFiles, resolveConfig)
}
