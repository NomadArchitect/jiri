...
func runResolve(jirix *jiri.X, args []string) error {
	...
	// instance ids between updated 'jiri.lock' and un-updated 'jiri.lock' file.
	// Jiri will halt when detecting conflicts in locks, so to make 'jiri resolve'
	// work, we need to temporarily disable the conflicts detection.
