...
func init() {
	...
	cmdUpload.Flags.BoolVar(&uploadMultipartFlag, "multipart", false, `Send multipart CL.  If this flag is set, upload will ignore -set-topic and will set a topic.`)
...
func runUpload(jirix *jiri.X, args []string) error {
	...
	// Always set topic when either topic is passed or this is a multipart CL.
	if uploadMultipartFlag || uploadTopicFlag != "" {
		setTopic = true
	}
