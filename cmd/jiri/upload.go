...
func runUpload(jirix *jiri.X, _ []string) error {
	...
	if !git.IsOnBranch() {
		return fmt.Errorf("The project is not on any branch. Aborting upload")
	}
	...
	if trackingBranch == "" {
		return fmt.Errorf("Current branch is un-tracked. Aborting upload")
	}
	remoteBranch := strings.Replace(trackingBranch, uploadRemoteNameFlag+"/", "", 1)
