...
func init() {
	...
	flags.StringVar(&deleteBranchFlags.branch, "branch", "", "Branch to delete.")
}

func runDeleteBranch(jirix *jiri.X, args []string) error {
	if deleteBranchFlags.branch == "" {
	...
	for key, state := range states {
		for _, branch := range state.Branches {
			if branch.Name == deleteBranchFlags.branch {
	...
	for key, bInfo := range projectMap {
		...
		if states[key].CurrentBranch.Name == deleteBranchFlags.branch {
		...
		if !deleteBranchFlags.dryRun {
			if err := git.DeleteBranch(deleteBranchFlags.branch, gitutil.ForceOpt(true)); err != nil {
