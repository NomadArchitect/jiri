...
func setDefaultBranchFlags() {
	...
	branchFlags.currentFlag = false
}

func createCommits(t *testing.T, fake *jiritest.FakeJiriRoot, localProjects []project.Project) {
...
func TestBranch(t *testing.T) {
	...
	defaultWant = fmt.Sprintf("%v%v(%v)\n", defaultWant, localProjects[i].Name, localProjects[i].Path)
	...
	currentBranchWant = fmt.Sprintf("%v%v(%v)\n", currentBranchWant, localProjects[i].Name, localProjects[i].Path)
	...
	gitLocals[i].CheckoutBranch(testBranch)
	currentBranchWant = fmt.Sprintf("%v%v(%v)\n", currentBranchWant, localProjects[i].Name, localProjects[i].Path)
	defaultWant = fmt.Sprintf("%v%v", defaultWant, currentBranchWant)
	...
	// Run with currentFlag
	branchFlags.currentFlag = true
	if got := executeBranch(t, fake, testBranch); !equalBranchOut(got, currentBranchWant) {
		t.Errorf("got %v, want %v", got, currentBranchWant)
	}
}
