...
func TestPrefixTree(t *testing.T) {
	...
	for _, v := range projects {
		if err := addToTree(fakeroot.X, &root, v, dropped); err != nil {
...
func TestGitModules(t *testing.T) {
	...
	if len(parsedScript) != len(pathMap) {
		t.Errorf("expecting %v set up commands, got %v", len(pathMap), len(parsedScript))
	}
	for k, v := range parsedScript {
		if p, ok := pathMap[k]; ok {
			if p.Revision != v {
	...
	if len(parsedModules) != len(pathMap) {
		t.Errorf("expecting %v submodules, got %v", len(pathMap), len(parsedModules))
	}
