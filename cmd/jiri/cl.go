...
func runCLCleanup(jirix *jiri.X, args []string) error {
	jirix.Logger.Warningf("jiri cl has been deprecated please use jiri upload. It would be removed soon.")
	if len(args) == 0 {
...
func initForMultiPart(jirix *jiri.X) (*multiPart, error) {
	...
	mp.currentBranch = states[mp.currentKey].CurrentBranch.Name
	if len(keys) == 1 {
...
func projectStates(jirix *jiri.X, allowdirty bool) (map[project.ProjectKey]*project.ProjectState, project.ProjectKeys, error) {
	...
	for _, s := range states {
		if s.CurrentBranch.Name == branch {
			key := s.Project.Key()
