...
func init() {
	...
	cmdUpdate.Flags.Var(&pkgsToSkipFlag, "package-to-skip", "Skip fetching this package. Repeatable.")
}
...
func runUpdate(jirix *jiri.X, args []string) error {
	...
	if len(args) > 0 {
		...
		if err := project.CheckoutSnapshot(jirix, args[0], gcFlag, runHooksFlag, fetchPkgsFlag, hookTimeoutFlag, fetchPkgsTimeoutFlag, pkgsToSkipFlag); err != nil {
			return err
		}
	} else {
		...
		err := project.UpdateUniverse(jirix, gcFlag, localManifestFlag,
			rebaseTrackedFlag, rebaseUntrackedFlag, rebaseAllFlag, runHooksFlag, fetchPkgsFlag, hookTimeoutFlag, fetchPkgsTimeoutFlag, pkgsToSkipFlag)
