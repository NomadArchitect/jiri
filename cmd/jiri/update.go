...
func init() {
	...
	cmdUpdate.Flags.BoolVar(&useLocalManifestFlag, "use-local-manifest", false, "Use local manifest")
	...
	cmdUpdate.Flags.BoolVar(&rebaseUntrackedFlag, "rebase-untracked", false, "Rebase untracked branches on to HEAD.")
}
...
func runUpdate(jirix *jiri.X, args []string) error {
	...
	if err := retry.Function(jirix.Context, func() error {
		if len(args) > 0 {
			...
			return project.UpdateUniverse(jirix, project.UpdateFlags{
				Gc:               gcFlag,
				ShowUpdateLogs:   verboseUpdateFlag,
				UseLocalManifest: useLocalManifestFlag,
	...
	return project.WriteUpdateHistorySnapshot(jirix, "", useLocalManifestFlag)
}
