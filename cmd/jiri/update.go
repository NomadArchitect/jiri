...
func runUpdate(jirix *jiri.X, args []string) error {
	...
	if attemptsFlag < 1 {
		return jirix.UsageErrorf("No of attempts should be more than equal to 1")
	}
	...
	var err error
	if len(args) > 0 {
		err = project.CheckoutSnapshot(jirix, args[0], gcFlag, hookTimeoutFlag)
	} else {
		err = project.UpdateUniverse(jirix, gcFlag, localManifestFlag,
			rebaseTrackedFlag, rebaseUntrackedFlag, rebaseAllFlag, hookTimeoutFlag)
	}
