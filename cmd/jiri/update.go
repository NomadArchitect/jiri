...
func init() {
	...
	cmdUpdate.Flags.BoolVar(&useLocalManifestFlag, "local-manifest", false, "Use local manifest")
...
func runUpdate(jirix *jiri.X, args []string) error {
	...
	if err := retry.Function(jirix.Context, func() error {
		if len(args) > 0 {
			...
			return project.UpdateUniverse(jirix, gcFlag, verboseUpdateFlag, useLocalManifestFlag, rebaseUntrackedFlag)
		}
