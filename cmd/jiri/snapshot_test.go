...
func writeReadme(t *testing.T, jirix *jiri.X, projectDir, message string) {
	...
	if err := gitutil.New(jirix.NewSeq(), gitutil.UserNameOpt("John Doe"), gitutil.UserEmailOpt("john.doe@example.com")).CommitFile(path, "creating README"); err != nil {
		t.Fatalf("%v", err)
	}
}
...
func TestSnapshot(t *testing.T) {
	...
	if err := project.UpdateUniverse(fake.X, project.UpdateFlags{Gc: true, ShowUpdateLogs: false, UseLocalManifest: false}); err != nil {
		t.Fatalf("%v", err)
	}
	...
	if err := project.UpdateUniverse(localX, project.UpdateFlags{Gc: true, ShowUpdateLogs: false, UseLocalManifest: false}); err != nil {
		t.Fatalf("%v", err)
	}
	for i, _ := range remoteProjects {
		localProject := filepath.Join(fake.X.Root, localProjectName(i))
