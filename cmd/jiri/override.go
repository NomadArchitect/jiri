...
func init() {
	cmdOverride.Flags.StringVar(&flagOverrideRevision, "revision", "", `Revision to check out for the remote.`)
	cmdOverride.Flags.StringVar(&flagOverrideGerritHost, "gerrithost", "", `The project Gerrit host.`)

	cmdOverride.Flags.BoolVar(&flagOverrideDelete, "delete", false, `Delete existing import. Import is matched using <manifest>, <remote> and name. <remote> is optional.`)
	cmdOverride.Flags.BoolVar(&flagOverrideList, "list", false, `List all the imports from .jiri_manifest. This flag doesn't accept any arguments. -json-out flag can be used to specify json output file.`)
...
Example:
  $ jiri override project path/to/project https://foo.com/bar.git

Run "jiri help manifest" for details on manifests.
`,
	ArgsName: "<name> <path> <remote>",
	ArgsLong: `
...
func runOverride(jirix *jiri.X, args []string) error {
	...
	} else if flagOverrideDelete && (len(args) < 1 || len(args) > 3) {
		return jirix.UsageErrorf("wrong number of arguments for the delete flag")
	} else if !flagOverrideDelete && !flagOverrideList && len(args) != 3 {
	...
	if flagOverrideDelete {
		...
		for _, p := range manifest.Overrides {
			if len(args) >= 3 && p.Remote != args[2] {
				overrides = append(overrides, p)
				continue
			}
			if len(args) >= 2 && p.Path != args[1] {
				overrides = append(overrides, p)
				continue
			}
			if p.Name != name {
		...
		if len(deleted) > 1 {
			return fmt.Errorf("more than one override matches the name, please provide path")
		...
		match := false
		for i, p := range manifest.Overrides {
			if p.Name == name && p.Path == path && p.Remote == remote {
