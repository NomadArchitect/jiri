...
Example:
  $ jiri override project https://foo.com/bar.git 0a0b0c
...
func runOverride(jirix *jiri.X, args []string) error {
	...
	if !manifestExists {
		return fmt.Errorf("'%s' does not exists", jirix.JiriManifestFile())
	}
	...
	if flagOverrideDelete {
		...
		if len(deleted) > 1 {
			return fmt.Errorf("more than 1 overrides matches the name, please provide path")
