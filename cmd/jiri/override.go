...
func init() {
	cmdOverride.Flags.StringVar(&overrideFlags.importManifest, "import-manifest", "", "Place holder")
...
func runOverride(jirix *jiri.X, args []string) error {
	...
	if overrideFlags.delete {
		...
		for _, p := range manifest.ProjectOverrides {
			overrideKeys[p.Name+project.KeySeparator+p.Remote] = true
		}
		for _, p := range manifest.ImportOverrides {
			overrideKeys[p.Name+project.KeySeparator+p.Remote] = true
		}
		if _, ok := overrideKeys[name+project.KeySeparator+remote]; !ok {
			if overrideFlags.importManifest != "" {
