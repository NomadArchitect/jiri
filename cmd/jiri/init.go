...
func init() {
	...
	cmdInit.Flags.StringVar(&rewriteSsoToHttpsFlag, "rewriteSsoToHttps", "", "Rewrites sso fetches, clones, etc to https. Takes true/false. This is temporary. It would be removed once we start using some library for this stuff.")
}
...
func runInit(env *cmdline.Env, args []string) error {
	...
	if rewriteSsoToHttpsFlag != "" {
		if val, err := strconv.ParseBool(rewriteSsoToHttpsFlag); err != nil {
			return fmt.Errorf("'rewriteSsoToHttps' flag should be true or false")
		} else {
