...
func init() {
	...
	cmdInit.Flags.StringVar(&forceDisableSubmodulesInfraOnly, "force-disable-submodules-infra-only", False, "Force disable submodules.")
...
func runInit(env *cmdline.Env, args []string) error {
	...
	// Override EnableSubmodules values with False if force disable flag is True
	if gitConfigSubmForce, err := jiri.GitGetConfig("jiri.forceDisableSubmodulesInfraOnly"); err == nil {
		if _, err := strconv.ParseBool(gitConfigSubmForce); err != nil {
			return fmt.Errorf("'jiri.forceDisableSubmomdulesInfraOnly' from git config should be true or false")
		}
		if gitConfigSubmForce {
			config.EnableSubmodules = gitConfigSubmForce
		}
	}
