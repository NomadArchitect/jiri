...
func init() {
	...
	cmdInit.Flags.StringVar(&keepGitHooks, "keep-git-hooks", "", "Stop updating git hooks in '.git/hooks' when doing 'jiri update'. Take true/false.")
}
...
func runInit(env *cmdline.Env, args []string) error {
	...
	if keepGitHooks != "" {
		if val, err := strconv.ParseBool(keepGitHooks); err != nil {
			return fmt.Errorf("'keep-current-git-hooks' flag should be true or false")
		} else {
