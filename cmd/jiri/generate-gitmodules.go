...
var cmdGenGitModule = &cmdline.Command{
	...
	Long: `
The "jiri gen-gitmodules <.gitmodule path>" command captures the current project state
...
func init() {
	...
	flags.StringVar(&genGitModuleFlags.genScript, "gen-script", "", "File to save generated git commands for seting up a superproject.")
}
...
func addToTree(jirix *jiri.X, root *prefixTree, proj project.Project, dropped project.Projects) error {
	...
	if len(pin.next) != 0 {
		// There is a project nested under proj.
...
func writeGitModules(jirix *jiri.X, projects project.Projects, outputPath string) error {
	...
	for i = 0; i < len(projEntries); i++ {
		if _, ok := dropped[projEntries[i].Key()]; ok {
			jirix.Logger.Debugf("dropped project %v+", projEntries[i])
			continue
		}
		decl, err := moduleDecl(projEntries[i], jirix.Root)
		if err != nil {
			return err
		}
		gitmoduleBuf.WriteString(decl)
		gitmoduleBuf.WriteString("\n")
		commandBuf.WriteString(commandDecl(projEntries[i]))
...
func moduleDecl(p project.Project, basepath string) (string, error) {
	tmpl := "[submodule \"%s\"]\n\tbranch = %s\n\tpath = %s\n\turl = %s"
	relPath, err := relativizePath(basepath, p.Path)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf(tmpl, p.Name, p.Revision, relPath, p.Remote), nil
}
