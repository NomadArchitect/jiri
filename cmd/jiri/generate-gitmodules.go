...
func (p *projectTree) addToTree(jirix *jiri.X, proj project.Project, dropped project.Projects) error {
	if p == nil {
	...
	if len(pin.children) != 0 {
		...
		if err := pin.prune(jirix, dropped); err != nil {
			return err
		}
		jirix.Logger.Debugf("\n")
	}
...
func (p *projectTree) prune(jirix *jiri.X, dropped project.Projects) error {
	...
	p.children = make(map[string]*projectTree)
	return nil
}
...
func writeGitModules(jirix *jiri.X, projects project.Projects, outputPath string) error {
	...
	for _, v := range projEntries {
		if err := root.addToTree(jirix, v, dropped); err != nil {
			return err
		}
	}
