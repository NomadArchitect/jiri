...
func TestDeleteBranch(t *testing.T) {
	...
	deleteBranchFlags.branch = testBranch
	...
	currentRevision, _ := gitLocals[i].CurrentRevision()
	want = fmt.Sprintf("%v\nProject %v(%v): Branch deleted. It might have left some dangling commits behind", want, localProjects[i].Name, relativePaths[i])
	want = fmt.Sprintf("%v\nTo restore it run git -C %q branch %v %v", want, relativePaths[i], testBranch, currentRevision)
...
func TestDeleteBranchDryRun(t *testing.T) {
	setDefaultDeleteBranchFlags()
	color.ColorFlag = false
	deleteBranchFlags.dryRun = true
	fake, cleanup := jiritest.NewFakeJiriRoot(t)
	defer cleanup()
	s := fake.X.NewSeq()
