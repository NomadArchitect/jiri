...
func init() {
	...
	cmdRunP.Flags.StringVar(&runpFlags.remotes, "remotes", "", "A Regular expression specifying projects to run commands in by matching against their remote URLs.")
}
...
func runRunp(jirix *jiri.X, args []string) error {
	...
	var keysRE, branchRE, remotesRE *regexp.Regexp
	var err error
	...
	if runpFlags.remotes != "" {
		remotesRE, err = regexp.Compile(runpFlags.remotes)
		if err != nil {
			return fmt.Errorf("failed to compile remotes regexp: %q: %v", runpFlags.remotes, err)
		}
	}
	...
	for _, localProject := range projects {
		...
		if remotesRE != nil && !remotesRE.MatchString(localProject.Remote) {
			continue
		}
