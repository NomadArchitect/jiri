...
func (g *Git) CloneMirror(url, path string) error {
	repo, err := git2go.Clone(url, path, &git2go.CloneOptions{
		Bare:                 true,
		RemoteCreateCallback: mirrorRemoteCallback,
	})
	if err != nil {
		return err
	}
...
func mirrorRemoteCallback(repo *git2go.Repository, remoteName, url string) (*git2go.Remote, git2go.ErrorCode) {
	remote, err := repo.Remotes.CreateWithFetchspec(remoteName, url, "+refs/*:refs/*")
	if err != nil {
	...
	err = config.SetBool(fmt.Sprintf("remote.%s.mirror", remoteName), true)
	if err != nil {
		return nil, git2go.ErrGeneric
	}
