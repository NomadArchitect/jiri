...
func TestfetchBinary(t *testing.T) {
	...
	tests := []struct {
		...
		{"linux-amd64", cipdVersionForTestA, digestMapA["linux-amd64"]},
		{"linux-arm64", cipdVersionForTestA, digestMapA["linux-arm64"]},
		{"linux-armv6l", cipdVersionForTestA, digestMapA["linux-armv6l"]},
		{"mac-amd64", cipdVersionForTestA, digestMapA["mac-amd64"]},
		{"linux-amd64", cipdVersionForTestB, digestMapB["linux-amd64"]},
		{"linux-arm64", cipdVersionForTestB, digestMapB["linux-arm64"]},
		{"linux-armv6l", cipdVersionForTestB, digestMapB["linux-armv6l"]},
		{"mac-amd64", cipdVersionForTestB, digestMapB["mac-amd64"]},
	}

	for _, test := range tests {
		cipdPath := path.Join(tmpDir, "cipd"+test.platform+test.version)
		if err := fetchBinary(cipdPath, test.platform, test.version, test.digest); err != nil {
			t.Errorf("failed to retrieve cipd binary for platform %q on version %q with digest %q: %v", cipdPlatform, cipdVersion, cipdVersionDigest, err)
		}
	}
}
...
func TestFetchDigest(t *testing.T) {
	...
	for _, platform := range tests {
		...
		if err != nil {
			t.Errorf("failed to retrieve cipd digest: %v", err)
		}
		if _, err := hex.DecodeString(digest); err != nil {
			t.Errorf("digest is not a valid hex string: %q", digest)
		}
	}
}

func TestSelfUpdateImpl(t *testing.T) {
	tmpDir, err := ioutil.TempDir("", "jiri-test")
	if err != nil {
