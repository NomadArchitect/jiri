...
func TestSupportedPlatform(t *testing.T) {
	if _, err := determinePlatform(); err != nil {
		t.Fatal(err)
	}
}
...
func testBootstrapCipdImpl(tmpDir, cipdPlatform, cipdVersion, cipdDigest string, t *testing.T) {
	...
	if err := bootstrapCipdImpl(cipdPath, cipdPlatform, cipdVersion, cipdDigest, "sha256"); err != nil {
...
func TestFetchCipdVersion(t *testing.T) {
	version, err := fetchFileFromGitiles(cipdVersionURL)
	if err != nil {
		t.Fatalf("failed to retrieve cipd version: %v", err)
	}
...
func TestFetchCipdDigest(t *testing.T) {
	digest, method, err := fetchCipdDigest("linux-amd64")
	if err != nil {
		t.Fatalf("failed to retrieve cipd digest: %v", err)
	}
	if method != "sha256" {
		t.Fatalf("unsupported hash method: %q", method)
	}
...
func TestCipdSelfUpdateImpl(t *testing.T) {
	platform, err := determinePlatform()
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := bootstrapCipdImpl(cipdPath, platform, cipdVersionForTestA, retrieveDigestA(platform), "sha256"); err != nil {
	...
	verified, err := verifyDigest(cipdData, retrieveDigestB(platform), "sha256")
	if err != nil {
		t.Fatal(err)
	}
	if !verified {
