...
func TestUpdatedUniverseEnabledSubmodules(t *testing.T) {
	...
	m.Projects = localProjects
	if err := fake.WriteRemoteManifest(m); err != nil {
		t.Fatal(err)
	}
	manifest, err := fake.ReadRemoteManifest()

	fmt.Printf("YupingDebugger: what remoteUnivese like %+v\n", manifest)

	// Make local manifest file matching local projects state.
	// if err := fake.WriteJiriManifest(&project.Manifest{
	// 	Imports: []project.Import{
	// 		{
	// 			Manifest: jiritest.ManifestFileName,
	// 			Name:     jiritest.ManifestProjectName,
	// 			Remote:   filepath.Join(fake.remote, jiritest.ManifestProjectPath),
	// 		},
	// 	},
	// }); err != nil {
	// 	t.Fatal(err)
