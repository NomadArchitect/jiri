...
func TestUpdateUniverseWhenLocalTracksLocal(t *testing.T) {
	...
	if err := project.UpdateUniverse(fake.X, false, false, false, false, true /*rebase-all*/, project.DefaultHookTimeout, true /*run-hooks*/); err != nil {
		t.Fatal(err)
	}
...
func TestRunHookFlag(t *testing.T) {
	...
	if err := project.UpdateUniverse(fake.X, false, false, true /*rebaseTracked*/, false, false, project.DefaultHookTimeout, false /*run-hooks*/); err != nil {
		t.Fatal(err)
	}
}
...
func TestUpdateWhenRemoteChangesRebased(t *testing.T) {
	...
	if err := project.UpdateUniverse(fake.X, false, false, true /*rebaseTracked*/, false, false, project.DefaultHookTimeout, true /*run-hooks*/); err != nil {
		t.Fatal(err)
	}
...
func testCheckoutSnapshot(t *testing.T, testURL bool) {
	...
	if testURL {
		...
		project.CheckoutSnapshot(fake.X, server.URL, false, project.DefaultHookTimeout, true /*run-hooks*/)
	} else {
		project.CheckoutSnapshot(fake.X, snapshotFile, false, project.DefaultHookTimeout, true /*run-hooks*/)
	}
...
func testLocalBranchesAreUpdated(t *testing.T, shouldLocalBeOnABranch, rebaseAll bool) {
	...
	if err := project.UpdateUniverse(fake.X, false, false, false, false, rebaseAll, project.DefaultHookTimeout, true /*run-hooks*/); err != nil {
		t.Fatal(err)
	}
...
func TestFileImportCycle(t *testing.T) {
	...
	err := project.UpdateUniverse(jirix, false, false, false, false, false, project.DefaultHookTimeout, true /*run-hooks*/)
...
func TestRemoteImportCycle(t *testing.T) {
	...
	// The update should complain about the cycle.
	err := project.UpdateUniverse(fake.X, false, false, false, false, false, project.DefaultHookTimeout, true /*run-hooks*/)
