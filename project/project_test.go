...
func TestOptionProjectsAndPackages(t *testing.T) {
	fake, cleanup := jiritest.NewFakeJiriRoot(t)
	defer cleanup()
	...
	createRemoteProj := func(i int, fetch, attributes string) {
		name := projectName(i)
		path := fmt.Sprintf("path-%d", i)
		if err := fake.CreateRemoteProject(name); err != nil {
	...
	for i := 0; i < numProjects; i++ {
		createRemoteProj(i, "true", "normal")
	}

	for i := numProjects; i < numProjects+numOptionalProjects; i++ {
		createRemoteProj(i, "false", "optional,debug")
	}
