...
// from a cache.
func testWithCache(t *testing.T, cacheType jiri.CacheType) {
	localProjects, fake, cleanup := setupUniverse(t)
	defer cleanup()
	s := fake.X.NewSeq()
...
func TestUpdateUniverseWithCache(t *testing.T) {
	testWithCache(t, jiri.CacheTypeReference)
}
...
func TestUpdateUniverseWithSharedCache(t *testing.T) {
	testWithCache(t, jiri.CacheTypeShared)
}
