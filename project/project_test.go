...
func assertDirExists(dirname string) error {
...
func assertFileExists(dirname string) error {
	fileInfo, err := os.Stat(dirname)
	if err != nil {
		return err
	}
...
func TestUpdateUniverseSimple(t *testing.T) {
	...
	for _, p := range localProjects {
		if err := assertDirExists(p.Path); err != nil {
...
func testWithCache(t *testing.T, shared bool) {
	...
	for _, p := range localProjects {
		...
		err := assertFileExists(p.Path + "/.git/objects/info/alternates")
...
func TestMoveNestedProjects(t *testing.T) {
	...
	checkReadme(t, fake.X, p, "nested folder")
	if err := assertDirExists(oldProjectPath); err == nil {
...
func TestUpdateUniverseMovedProject(t *testing.T) {
	...
	if err := assertDirExists(localProjects[2].Path); err != nil {
...
func testUpdateUniverseDeletedProject(t *testing.T, testDirtyProjectDelete bool) {
	...
	for i := 1; i <= 5; i++ {
		if err := assertDirExists(localProjects[i].Path); err != nil {
	...
	for i := 1; i <= 5; i++ {
		err := assertDirExists(localProjects[i].Path)
...
func TestIgnoredProjectsNotDeleted(t *testing.T) {
	...
	if err := assertDirExists(localProjects[1].Path); err != nil {
