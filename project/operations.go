...
func (op createOperation) checkoutProject(jirix *jiri.X, cache string) error {
	...
	if op.destination == jirix.Root {
		...
		if jirix.Partial {
			...
			if err := scm.Config("remote.origin.promisor", "true"); err != nil {
				return err
			}
			if err := scm.Config("remote.origin.partialCloneFilter", "blob:none"); err != nil {
				return err
			}
		}
		...
		if cache != "" {
			...
			if err := ioutil.WriteFile(path.Join(op.destination, ".git/objects/info/alternates"), []byte(path.Join(cache, objPath) + "\n"), 0644); err != nil {
				return err
			}
		}
	...
	if jirix.Partial && cache != "" {
		...
		if err := scm.Config("remote.cache.partialCloneFilter", "blob:none"); err != nil {
			return err
		}
		if err := scm.Config("remote.cache.url", cache); err != nil {
			return err
		}
		if err := scm.Config("remote.cache.promisor", "true"); err != nil {
			return err
		}
	}
