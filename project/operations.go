...
func (op createOperation) checkoutProject(jirix *jiri.X, cache string) error {
	...
	if op.project.HistoryDepth > 0 {
		// Checkout directly from remote for shallow projects
		err = clone(jirix, remote, op.destination, gitutil.NoCheckoutOpt(true), gitutil.DepthOpt(op.project.HistoryDepth))
	} else {
