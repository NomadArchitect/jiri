	...
	if vals[0] == 2 { // Move
		if vals[1] == 4 { // Update
			if strings.HasPrefix(ops[i].Source(), ops[j].Source()+string(filepath.Separator)) { // Move is in a child project of Update
				if strings.HasPrefix(ops[i].Source(), ops[i].Destination()+string(filepath.Separator)) { // Move out
					return false // Move happens after update
				}
			}
		}
		if vals[1] == 3 { // Create
			if strings.HasPrefix(ops[i].Destination(), ops[j].Destination()+string(filepath.Separator)) { // Create is the parent of the move destination
				return false // Move happens after crease
			}
		}
		if vals[1] == 2 { // Move
			if strings.HasPrefix(ops[i].Destination(), ops[i].Source()+string(filepath.Separator)) { // Move out
				return true
			} else if strings.HasPrefix(ops[i].Source(), ops[i].Destination()+string(filepath.Separator)) { // Move in
				return false
			}
		}
	}
