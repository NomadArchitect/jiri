...
// removeSubmoduleBranches removes initial branches from submoduels
// We create a local foo branch in all submodules first before running update.
// If submodules were created for the first time, "local-foo" branch would not exist. We remove all branches.
// Otherwise, submodules pre-exist the update, then we remove on the dummy branch.
func removeSubmoduleBranches(jirix *jiri.X, superproject Project, fooBranch string) error {
	...
	for _, subm := range submStates {
		...
		scm := gitutil.New(jirix, gitutil.RootDirOpt(subm.Path))
		if exist, _ := scm.CheckBranchExists(fooBranch); !exist {
			...
			if err := scm.DeleteBranch(fooBranch); err != nil {
				return err
			}
		}
	}
