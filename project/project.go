...
func runDeleteOperations(jirix *jiri.X, ops []deleteOperation) error {
	...
	for _, op := range ops {
		...
		taskId := jirix.Logger.AddTaskMsg("Trying to delete project %s", op.Project().Name)
		jirix.Logger.Debugf("%s", op)
		if err := op.Run(jirix); err != nil {
			jirix.Logger.RemoveTaskMsg(taskId)
			return fmt.Errorf("Deleting project %q: %s", op.Project().Name, err)
...
func runMoveOperations(jirix *jiri.X, ops []moveOperation) error {
	...
	for _, op := range ops {
		...
		taskId := jirix.Logger.AddTaskMsg("Moving project %s", op.Project().Name)
		jirix.Logger.Debugf("%s", op)
		if err := op.Run(jirix); err != nil {
			jirix.Logger.RemoveTaskMsg(taskId)
...
func runCommonOperations(jirix *jiri.X, ops operations) error {
	for _, op := range ops {
		taskId := jirix.Logger.AddTaskMsg("Updating project %s", op.Project().Name)
		jirix.Logger.Debugf("%s", op)
		if err := op.Run(jirix); err != nil {
			jirix.Logger.RemoveTaskMsg(taskId)
...
func runHooks(jirix *jiri.X, ops []operation, hooks Hooks, runHookTimeout uint) error {
	...
	for _, hook := range hooks {
		...
		go func(hook Hook) {
			...
			fmt.Fprintf(outFile, "%s: output for hook(%v) for project %q", jirix.Color.Cyan("DEBUG: "), hook.Name, hook.ProjectName)
