...
func CheckoutSnapshot(jirix *jiri.X, snapshot string, gc bool) error {
	...
	if err := updateProjects(jirix, localProjects, remoteProjects, hooks, gc, false /*rebaseUntracked*/, true /*snapshot*/); err != nil {
		return err
	}
...
func UpdateUniverse(jirix *jiri.X, gc bool, localManifest bool, rebaseUntracked bool) (e error) {
	...
	updateFn := func(scanMode ScanMode) error {
		...
		return updateProjects(jirix, localProjects, remoteProjects, hooks, gc, rebaseUntracked, false /*snapshot*/)
	}
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, gc bool, rebaseUntracked bool, snapshot bool) error {
...
func runHooks(jirix *jiri.X, ops []operation, hooks Hooks) error {
	...
	showHookOutput := log.DebugVerboseFlag || log.TraceVerboseFlag || log.AllVerboseFlag
	for _, hook := range hooks {
		...
		go func(hook Hook) {
			...
			if err := jirix.NewSeq().Verbose(showHookOutput).CaptureAll(outFile, errFile).Dir(hook.ActionPath).Timeout(5 * time.Minute).Last(filepath.Join(hook.ActionPath, hook.Action)); err != nil {
