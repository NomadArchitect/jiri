...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, gc bool, showUpdateLogs bool) error {
	...
	localStates := make(map[ProjectKey]*ProjectState, len(localProjects))
	go func() {
	...
	ops := computeOperations(localProjects, remoteProjects, localStates, gc)
...
// projects.
func computeOperations(localProjects, remoteProjects Projects, localProjectsStates map[ProjectKey]*ProjectState, gc bool) operations {
	result := operations{}
	...
	for key, _ := range allProjects {
		...
		if state, ok := localProjectsStates[key]; ok {
			localState = state
		}
		result = append(result, computeOp(local, remote, localState, gc))
	}
	sort.Sort(result)
	return result
}

func computeOp(local, remote *Project, localState *ProjectState, gc bool) operation {
	switch {
	...
	case local != nil && remote != nil:
		...
		case localState.CurrentBranch == "" && local.Revision == remote.Revision:
			return nullOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
			}}
		case local.Revision == localState.CurrentTrackingBranchRev:
			return nullOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
