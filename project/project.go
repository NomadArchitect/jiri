...
func checkoutHeadRevision(jirix *jiri.X, project Project, forceCheckout bool) error {
	...
	if project.Revision != "" && project.Revision != "HEAD" {
		//might be a tag
		if err2 := gitutil.New(jirix, gitutil.RootDirOpt(project.Path)).Fetch("origin", gitutil.FetchTagOpt(project.Revision)); err2 != nil {
			// error while fetching tag, return original err and debug lof this err
