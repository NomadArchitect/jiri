...
func setProjectRevisions(jirix *jiri.X, projects Projects) (Projects, error) {
	...
	tempProjectsMap := make(map[ProjectKey]*Project)
	for name, project := range projects {
		wg.Add(1)
		// Cannot directly use address of project as underlying onject changes in the loop
		projectCopy := project
		tempProjectsMap[name] = &projectCopy
	...
	wg.Wait()
	if routineError != nil {
		return nil, routineError
	}
	for name, project := range tempProjectsMap {
