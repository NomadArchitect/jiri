...
func updateOrCreateCache(jirix *jiri.X, dir, remote, branch string, depth int) error {
	...
	updateCache := func() error {
		...
		if err := scm.Config("--replace-all", "remote.origin.fetch", "+refs/heads/*:refs/heads/*"); err != nil {
	...
	createCacheThroughBundle := func() error {
		fetched, bundlePath, err := gerrit.FetchCloneBundle(jirix, remote, dir)
		if !fetched {
			if err != nil {
				return err
			}
			return gerrit.ErrCloneBundleNotAvailable
		}
