...
func CheckoutSnapshot(jirix *jiri.X, snapshot string, gc bool) error {
	...
	if err := updateProjects(jirix, localProjects, remoteProjects, hooks, gc, true /*showUpdateLogs*/, false /*rebaseUntracked*/, false /*rebaseAll*/); err != nil {
		return err
	}
...
func PollProjects(jirix *jiri.X, projectSet map[string]struct{}) (_ Update, e error) {
	...
	ops := computeOperations(localProjects, remoteProjects, states, false /*gc*/, false /*rebaseUntracked*/, false /*rebaseAll*/)
	s := jirix.NewSeq()
	for _, op := range ops {
...
// removed.
func UpdateUniverse(jirix *jiri.X, gc bool, showUpdateLogs bool, localManifest bool, rebaseUntracked bool, rebaseAll bool) (e error) {
	s := jirix.NewSeq()
	...
	updateFn := func(scanMode ScanMode) error {
		...
		return updateProjects(jirix, localProjects, remoteProjects, hooks, gc, showUpdateLogs, rebaseUntracked, rebaseAll)
	}
...
func syncProjectMaster(jirix *jiri.X, project Project, state ProjectState, showUpdateLogs, rebaseUntracked, rebaseAll bool) error {
	s := jirix.NewSeq()
	...
	for _, branch := range branches {
		if branch.Tracking != nil { // tracked branch
			...
			} else if state.CurrentBranch.Name == branch.Name || showUpdateLogs {
				s.Verbose(true).Output([]string{
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, gc bool, showUpdateLogs bool, rebaseUntracked bool, rebaseAll bool) error {
	...
	ops := computeOperations(localProjects, remoteProjects, states, gc, rebaseUntracked, rebaseAll)
...
func (op moveOperation) Run(jirix *jiri.X, showUpdateLogs bool) error {
	...
	if err := syncProjectMaster(jirix, op.project, op.state, showUpdateLogs, op.rebaseUntracked, op.rebaseAll); err != nil {
		return err
	}
...
// projects.
func computeOperations(localProjects, remoteProjects Projects, states map[ProjectKey]*ProjectState, gc, rebaseUntracked, rebaseAll bool) operations {
	result := operations{}
	...
	for key, _ := range allProjects {
		...
		result = append(result, computeOp(local, remote, state, gc, rebaseUntracked, rebaseAll))
	}
	sort.Sort(result)
	return result
}

func computeOp(local, remote *Project, state *ProjectState, gc, rebaseUntracked, rebaseAll bool) operation {
	switch {
