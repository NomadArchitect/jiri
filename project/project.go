...
func fetchAll(jirix *jiri.X, project Project) error {
	...
	if project.HistoryDepth > 0 {
		...
		if project.Tag != "" {
			return fetch(jirix, project.Path, "origin", gitutil.FetchTagOpt(project.Tag))
		}
	}
	return nil
}
...
func (ld *loader) load(jirix *jiri.X, root, file string, localManifest bool) error {
	...
	for _, remote := range m.Imports {
		...
		if !ok {
			...
			if err := clone(jirix, p.Remote, path, gitutil.NoCheckoutOpt(true)); err != nil {
				return err
			}
			p.Revision = "HEAD"
...
func createOrUpdateCacheAt(jirix *jiri.X, dir, remote string, depth int, branch string) error {
	if isPathDir(dir) {
		...
		} else if err := fetch(jirix, dir, "origin", gitutil.PruneOpt(true)); err != nil {
			return err
		}
	} else {
		// Create cache
		...
		defer task.Done()
		if err := retry.Function(jirix, func() error {
			return gitutil.New(jirix).CloneMirror(remote, dir, depth)
		}, fmt.Sprintf("CloneMirror %s", remote),
			retry.AttemptsOpt(jirix.Attempts)); err != nil {
			return err
		}
	}
...
func updateCache(jirix *jiri.X, remoteProjects Projects) error {
	...
	workQueue := make(chan *work, len(remoteProjects))

	// recurse through projects, get cache dir and put unique cache dirs along
	// with their projects inside workQueue for processing
	for _, project := range remoteProjects {
		if cacheDirPath, err := project.CacheDirPath(jirix); err == nil {
			...
			workQueue <- &work{project, cacheDirPath}
		} else {
			errs <- err
		}
	...
	// Create jirix.Jobs number of go routines to process workQueue
	for i := uint(0); i < jirix.Jobs; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			for w := range workQueue {
				...
				if err := createOrUpdateCacheAt(jirix, w.cacheDirPath, project.Remote, project.HistoryDepth, project.RemoteBranch); err != nil {
					errs <- err
				}
			}
		}()
...
func (op createOperation) Run(jirix *jiri.X) (e error) {
	...
	if jirix.Shared && cache != "" {
		...
		if err := clone(jirix, op.project.Remote, tmpDir, gitutil.ReferenceOpt(ref),
			gitutil.NoCheckoutOpt(true), gitutil.DepthOpt(op.project.HistoryDepth)); err != nil {
			return err
		}
	}
