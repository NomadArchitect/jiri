...
type Project struct {
	...
	// This is used to stroe if local branch of the project
	LocalBranch            string `xml:"-"`
...
func setProjectRevisions(jirix *jiri.X, projects Projects) (Projects, error) {
	...
	for name, project := range projects {
		...
		if git.IsOnBranch() {
			if _, localBranch, err := git.GetBranches(); err != nil {
				return nil, err
			} else {
				project.LocalBranch = localBranch
			}
		}
...
func createAndManageCache(jirix *jiri.X, remoteProjects Projects) error {
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, gc bool, showUpdateLogs bool) error {
	...
	go func() {
		defer wg.Done()
		if err := createAndManageCache(jirix, remoteProjects); err != nil {
			errs <- err
			return
		}
...
func (op createOperation) Run(jirix *jiri.X) (e error) {
	...
	if err := gitutil.New(s).Clone(op.project.Remote, tmpDir, cache, jirix.IsCacheShared); err != nil {
		return err
	}
	cwd, err := os.Getwd()
	if err != nil {
...
func computeOp(local, remote *Project, gc bool) operation {
	...
	case local != nil && remote != nil:
		...
		case local.LocalBranch == "" && local.Revision == remote.Revision:
