		...
		if tracking != nil { // tracked branch
			...
			if rebaseSuccess {
				...
				if project.GitSubmodules && jirix.EnableSubmodules {
					...
					if err := scm.CheckoutBranch(branch.Name, (project.GitSubmodules && jirix.EnableSubmodules)); err != nil {
						msg := fmt.Sprintf("For project %s(%s), not able to update submodules", project.Name, relativePath)
						jirix.Logger.Errorf(msg)
						jirix.IncrementFailures()
						continue
					}
			...
			if rebaseUntracked {
				...
				if rebaseSuccess {
					...
					if project.GitSubmodules && jirix.EnableSubmodules {
						...
						if err := scm.CheckoutBranch(branch.Name, (project.GitSubmodules && jirix.EnableSubmodules)); err != nil {
							msg := fmt.Sprintf("For project %s(%s), not able to update superproject after rebasing untracked branch", project.Name, relativePath)
							jirix.Logger.Errorf(msg)
							jirix.IncrementFailures()
							continue
						}
				...
				msg += fmt.Sprintf("\nTo update submoduels, please run 'git submodule update' as fuchsia root.")
				msg += fmt.Sprintf("\n%s\n\n", gitCommand)
