...
func loadManifestFile(jirix *jiri.X, file string, localProjects Projects, useLocalManifest bool) (Projects, Hooks, error) {
	ld := newManifestLoader(localProjects, false)
	if err := ld.Load(jirix, "", file, "", useLocalManifest); err != nil {
		return nil, nil, err
	}
	return ld.Projects, ld.Hooks, nil
}

func loadUpdatedManifest(jirix *jiri.X, localProjects Projects, useLocalManifest bool) (Projects, Hooks, string, error) {
	jirix.TimerPush("load updated manifest")
	defer jirix.TimerPop()
	ld := newManifestLoader(localProjects, true)
	if err := ld.Load(jirix, "", jirix.JiriManifestFile(), "", useLocalManifest); err != nil {
...
// removed.
func UpdateUniverse(jirix *jiri.X, gc bool, showUpdateLogs bool, useLocalManifest bool, rebaseUntracked bool) (e error) {
	s := jirix.NewSeq()
	...
	updateFn := func(scanMode ScanMode) error {
		...
		remoteProjects, hooks, tmpLoadDir, err := loadUpdatedManifest(jirix, localProjects, useLocalManifest)
		matchLocalWithRemote(localProjects, remoteProjects)
...
func (ld *loader) loadNoCycles(jirix *jiri.X, root, file, cycleKey string, useLocalManifest bool) error {
	...
	if err := ld.load(jirix, root, file, useLocalManifest); err != nil {
		return err
	}
...
func (ld *loader) Load(jirix *jiri.X, root, file, cycleKey string, useLocalManifest bool) error {
	jirix.TimerPush("load " + shortFileName(jirix.Root, file))
	defer jirix.TimerPop()
	return ld.loadNoCycles(jirix, root, file, cycleKey, useLocalManifest)
}

func (ld *loader) load(jirix *jiri.X, root, file string, useLocalManifest bool) error {
	...
	for _, remote := range m.Imports {
		...
		if err := ld.resetAndLoad(jirix, nextRoot, nextFile, remote.cycleKey(), p, useLocalManifest); err != nil {
			return err
		}
	}
	...
	for _, local := range m.LocalImports {
		...
		if err := ld.Load(jirix, root, nextFile, "", useLocalManifest); err != nil {
			return err
		}
	}
...
func (ld *loader) resetAndLoad(jirix *jiri.X, root, file, cycleKey string, project Project, useLocalManifest bool) (e error) {
	if useLocalManifest {
		return ld.Load(jirix, root, file, cycleKey, useLocalManifest)
	}
