...
func syncProjectMaster(jirix *jiri.X, project Project, state ProjectState, showUpdateLogs bool) error {
	...
	for _, branch := range state.Branches {
		if branch.TrackingBranch != nil { // tracked branch
			if branch.Revision == branch.TrackingBranch.Revision {
				continue
			}

			if err := git.CheckoutBranch(branch.Name); err != nil {
				s.Verbose(true).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), not able to rebase your local branch %q onto %q.", project.Name, branch.Name, branch.TrackingBranch.Name),
					"Please do it manually.",
				})
				continue
			}
			rebaseSuccess, err := tryRebase(jirix, project, branch.TrackingBranch.Name)
			if err != nil {
				return err
			}
			if rebaseSuccess {
				s.Verbose(showUpdateLogs).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), rebased your local branch %q on %q", project.Name, branch.Name, branch.TrackingBranch.Name),
				})
			} else {
				s.Verbose(true).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), not able to rebase your local branch %q onto %q.", project.Name, branch.Name, branch.TrackingBranch.Name),
...
func computeOp(local, remote *Project, state *ProjectState, gc bool) operation {
	...
	case local != nil && remote != nil:
		...
		case state.CurrentBranch.TrackingBranch == nil && local.Revision != remote.Revision:
			return updateOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
