...
func CreateSnapshot(jirix *jiri.X, file string, useLocalManifest bool) error {
	...
	_, hooks, err := loadManifestFile(jirix, jirix.JiriManifestFile(), localProjects, useLocalManifest)
	if err != nil {
		return err
	}
...
func CheckoutSnapshot(jirix *jiri.X, snapshot string, gc bool) error {
	...
	if err := updateProjects(jirix, localProjects, remoteProjects, hooks, UpdateFlags{Gc: gc, ShowUpdateLogs: true}); err != nil {
		return err
	}
...
func loadManifestFile(jirix *jiri.X, file string, localProjects Projects, useLocal bool) (Projects, Hooks, error) {
	ld := newManifestLoader(localProjects, false)
	if err := ld.Load(jirix, "", file, "", useLocal); err != nil {
		return nil, nil, err
	}
	return ld.Projects, ld.Hooks, nil
}

func loadUpdatedManifest(jirix *jiri.X, localProjects Projects, useLocal bool) (Projects, Hooks, string, error) {
	jirix.TimerPush("load updated manifest")
	defer jirix.TimerPop()
	ld := newManifestLoader(localProjects, true)
	if err := ld.Load(jirix, "", jirix.JiriManifestFile(), "", useLocal); err != nil {
...
// removed.
func UpdateUniverse(jirix *jiri.X, updateFlags UpdateFlags) (e error) {
	s := jirix.NewSeq()
	...
	updateFn := func(scanMode ScanMode) error {
		...
		remoteProjects, hooks, tmpLoadDir, err := loadUpdatedManifest(jirix, localProjects, updateFlags.UseLocalManifest)
		matchLocalWithRemote(localProjects, remoteProjects)
		...
		return updateProjects(jirix, localProjects, remoteProjects, hooks, updateFlags)
	}
...
func WriteUpdateHistorySnapshot(jirix *jiri.X, snapshotPath string, useLocalManifest bool) error {
	seq := jirix.NewSeq()
	snapshotFile := filepath.Join(jirix.UpdateHistoryDir(), time.Now().Format(time.RFC3339))
	if err := CreateSnapshot(jirix, snapshotFile, useLocalManifest); err != nil {
		return err
	}
...
func syncProjectMaster(jirix *jiri.X, project Project, updateFlags UpdateFlags) error {
	s := jirix.NewSeq()
	git := gitutil.New(s, gitutil.RootDirOpt(project.Path))
	if !git.IsOnBranch() {
		...
		if trackingBranch != "" {
			...
			if rebaseSuccess {
				s.Verbose(updateFlags.ShowUpdateLogs).Output([]string{fmt.Sprintf("NOTE: For project (%v), rebased your local branch %v on %v", project.Name, branch, trackingBranch)})
			} else {
			...
			if updateFlags.RebaseUntracked {
				rebaseSuccess, err := tryRebase(jirix, project, revision)
				if err != nil {
					return err
				}
				if rebaseSuccess {
					s.Verbose(updateFlags.ShowUpdateLogs).Output([]string{fmt.Sprintf("NOTE: For project (%v), rebased your untracked branch %v on %v", project.Name, branch, revision)})
				} else {
					s.Verbose(true).Output([]string{
...
func (ld *loader) loadNoCycles(jirix *jiri.X, root, file, cycleKey string, useLocal bool) error {
	...
	if err := ld.load(jirix, root, file, useLocal); err != nil {
		return err
	}
...
func (ld *loader) Load(jirix *jiri.X, root, file, cycleKey string, useLocal bool) error {
	jirix.TimerPush("load " + shortFileName(jirix.Root, file))
	defer jirix.TimerPop()
	return ld.loadNoCycles(jirix, root, file, cycleKey, useLocal)
}

func (ld *loader) load(jirix *jiri.X, root, file string, useLocal bool) error {
	...
	for _, remote := range m.Imports {
		...
		if err := ld.resetAndLoad(jirix, nextRoot, nextFile, remote.cycleKey(), p, useLocal); err != nil {
			return err
		}
	}
	...
	for _, local := range m.LocalImports {
		...
		if err := ld.Load(jirix, root, nextFile, "", useLocal); err != nil {
			return err
		}
	}
...
func (ld *loader) resetAndLoad(jirix *jiri.X, root, file, cycleKey string, project Project, useLocal bool) (e error) {
	if useLocal {
		return ld.Load(jirix, root, file, cycleKey, useLocal)
	}
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, updateFlags UpdateFlags) error {
	...
	ops := computeOperations(localProjects, remoteProjects, states, updateFlags.Gc)
	...
	s := jirix.NewSeq()
	for _, op := range ops {
		updateFn := func() error { return op.Run(jirix, updateFlags) }
		if err := s.Verbose(updateFlags.ShowUpdateLogs).Call(updateFn, "%v", op).Done(); err != nil {
			return fmt.Errorf("error updating project %q: %v", op.Project().Name, err)
		}
	}
	if err := runHooks(jirix, ops, hooks, updateFlags.ShowUpdateLogs); err != nil {
		return err
	}
...
type operation interface {
	...
	Run(jirix *jiri.X, updateFlags UpdateFlags) error
...
func (op createOperation) Run(jirix *jiri.X, updateFlags UpdateFlags) (e error) {
	s := jirix.NewSeq()
...
func (op deleteOperation) Run(jirix *jiri.X, updateFlags UpdateFlags) error {
	s := jirix.NewSeq()
	if op.gc {
...
func (op moveOperation) Run(jirix *jiri.X, updateFlags UpdateFlags) error {
	s := jirix.NewSeq()
	path, perm := filepath.Dir(op.destination), os.FileMode(0755)
	if err := s.MkdirAll(path, perm).Rename(op.source, op.destination).Done(); err != nil {
		return err
	}
	if err := syncProjectMaster(jirix, op.project, updateFlags); err != nil {
		return err
	}
...
func (op updateOperation) Run(jirix *jiri.X, updateFlags UpdateFlags) error {
	if err := syncProjectMaster(jirix, op.project, updateFlags); err != nil {
		return err
	}
	return writeMetadata(jirix, op.project, op.project.Path)
...
func (op nullOperation) Run(jirix *jiri.X, updateFlags UpdateFlags) error {
	return writeMetadata(jirix, op.project, op.project.Path)
}
