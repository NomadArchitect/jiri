...
// FilterOptionalProjectsPackages removes projects and packages that have fetch="false" based
// on the attributes in attrs.
func FilterOptionalProjectsPackages(attrs string, projects Projects, pkgs Packages) (Projects, Packages) {
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, pkgs Packages, gc bool, runHookTimeout, fetchTimeout uint, rebaseTracked, rebaseUntracked, rebaseAll, snapshot, shouldRunHooks, shouldFetchPkgs bool) error {
	...
	oldPkgs := pkgs
	remoteProjects, pkgs = FilterOptionalProjectsPackages(jirix.FetchingAttrs, remoteProjects, pkgs)
