...
func CreateSnapshot(jirix *jiri.X, file string, hooks Hooks, packs Packages, localManifest bool) error {
	...
	if hooks == nil || packs == nil {
		if _, tmpHooks, tmpPackages, err := LoadManifestFile(jirix, jirix.JiriManifestFile(), localProjects, localManifest); err != nil {
			return err
		} else {
			if hooks == nil {
				hooks = tmpHooks
...
func CheckoutSnapshot(jirix *jiri.X, snapshot string, gc, runHooks, runPackages bool, runHookTimeout, runPackagesTimeout uint) error {
	...
	remoteProjects, hooks, packs, err := LoadSnapshotFile(jirix, snapshot)
	if err != nil {
		return err
	}
	if err := updateProjects(jirix, localProjects, remoteProjects, hooks, packs, gc, runHookTimeout, runPackagesTimeout, false /*rebaseTracked*/, false /*rebaseUntracked*/, false /*rebaseAll*/, true /*snapshot*/, runHooks, runPackages); err != nil {
		return err
	}
	return WriteUpdateHistorySnapshot(jirix, snapshot, hooks, packs, false)
}
...
// removed.
func UpdateUniverse(jirix *jiri.X, gc, localManifest, rebaseTracked, rebaseUntracked, rebaseAll, runHooks, runPackages bool, runHookTimeout, runPackagesTimeout uint) (e error) {
	...
	updateFn := func(scanMode ScanMode) error {
		...
		remoteProjects, hooks, packs, err := LoadUpdatedManifest(jirix, localProjects, localManifest)
		...
		return updateProjects(jirix, localProjects, remoteProjects, hooks, packs, gc, runHookTimeout, runPackagesTimeout, rebaseTracked, rebaseUntracked, rebaseAll, false /*snapshot*/, runHooks, runPackages)
	}
...
func WriteUpdateHistorySnapshot(jirix *jiri.X, snapshotPath string, hooks Hooks, packs Packages, localManifest bool) error {
	snapshotFile := filepath.Join(jirix.UpdateHistoryDir(), time.Now().Format(time.RFC3339))
	if err := CreateSnapshot(jirix, snapshotFile, hooks, packs, localManifest); err != nil {
		return err
	}
...
func updateProjects(jirix *jiri.X, localProjects, remoteProjects Projects, hooks Hooks, packs Packages, gc bool, runHookTimeout, runPackagesTimeout uint, rebaseTracked, rebaseUntracked, rebaseAll, snapshot, shouldRunHooks, shouldFetchPackages bool) error {
	...
	if shouldFetchPackages && len(packs) > 0 {
		if err := RunPackages(jirix, packs, runPackagesTimeout); err != nil {
			return err
		}
	}
