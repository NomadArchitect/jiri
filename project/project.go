...
// HostnameAllowed determines if hostname is allowed under reference. Due to
// the fact that domain names are resolved in reverse order, this fuction
// matches each character in reverse order to allow hierarchical matching.
...
// HostnameAllowedWildcard determines if hostname is allowed under reference.
// It allows wildcard matching using '*'.
func HostnameAllowedWildcard(reference, hostname string) bool {
	refIdx := 0
	starIdx := -1
	hostIdx := 0
	lastStarMatchIdx := 0

	for hostIdx < len(hostname) {
		if refIdx > len(reference)-1 {
			if starIdx == -1 {
				return false
			}
			refIdx = starIdx + 1
			lastStarMatchIdx++
			hostIdx = lastStarMatchIdx
			continue
		}
		if reference[refIdx] == hostname[hostIdx] {
			refIdx++
			hostIdx++
		} else if reference[refIdx] != '*' {
			if starIdx == -1 {
				return false
			}
			refIdx = starIdx + 1
			...
			starIdx = refIdx
			lastStarMatchIdx = hostIdx
			refIdx++
		}
	}

	if refIdx != len(reference) {
		for i := refIdx; i < len(reference); i++ {
			if reference[i] != '*' {
				return false
			}
		}
