...
type importCache struct {
	...
	// message incase of import conflict
	parentImport string
}
	...
	if v, ok := ld.importCacheMap[strings.Trim(project.Remote, "/")]; ok {
		...
		if !lm && ref != "JIRI_HEAD" {
			if tref, err := GetHeadRevision(jirix, project); err != nil {
				...
				return fmt.Errorf("Conflicting ref for import %s - %q and %q in file:\n'%s' for imports:\n'%s' and '%s'\n\n",
					jirix.Color.Red(project.Remote), ref, tref, jirix.Color.Yellow(ld.parentFile), jirix.Color.Yellow(v.parentImport),
					jirix.Color.Yellow(parentImport))
			}
		}
	} else {
