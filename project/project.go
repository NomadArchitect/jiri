...
type Project struct {
	...
	// This is used to stroe if local project is on a branch
	IsOnBranch             bool   `xml:"-"`
	LocalTrackingBranch    string `xml:"-"`
	LocalTrackingBranchRev string `xml:"-"`
}
...
func setProjectRevisions(jirix *jiri.X, projects Projects) (Projects, error) {
	...
	for name, project := range projects {
		...
		project.IsOnBranch = git.IsOnBranch()
...
func getTrackingBranchInfo(jirix *jiri.X, localProjects Projects) error {
	...
	modifiedProjects := make(chan Project, len(localProjects))
	...
	for project := range modifiedProjects {
		localProjects[project.Key()] = project
	}
	return nil
}
...
func runHooks(jirix *jiri.X, ops []operation, hooks Hooks) error {
	...
	defer jirix.TimerPop()
	routineOutputs := make(chan struct {
		outReader *os.File
		errReader *os.File
		err       error
	}, len(hooks))
	...
	for _, hook := range hooks {
		...
		wg.Add(1)
		go func(hook Hook, outWriter, errWriter, outReader, errReader *os.File) {
			defer func() {
				wg.Done()
				outWriter.Close()
				errWriter.Close()
		...
		}(hook, outWriter, errWriter, outReader, errReader)

	}
	wg.Wait()
	close(routineOutputs)
	multiErr := make(MultiError, 0)
	for out := range routineOutputs {
...
func computeOp(local, remote *Project, gc bool) operation {
	...
	case local != nil && remote != nil:
		...
		case !local.IsOnBranch && local.Revision == remote.Revision:
			return nullOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
			}}
		case local.Revision == local.LocalTrackingBranchRev:
			return nullOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
