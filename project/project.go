...
func syncProjectMaster(jirix *jiri.X, project Project, state ProjectState, showUpdateLogs bool) error {
	s := jirix.NewSeq()
	...
	if uncommitted, err := git.HasUncommittedChanges(); err != nil {
		...
		line1 := fmt.Sprintf("Note: Project %q contains uncommited changes.", project.Name)
	...
	if state.CurrentBranch.Name == "" {
		...
		defer func() {
			if err := checkoutHeadRevision(jirix, project, false); err != nil {
				...
				panic(fmt.Sprintf("for project %q, something went very wrong", project.Name, err))
			}
		}()
	} else {
		defer func() {
		...
		if branch.TrackingBranch != "" {
			...
			if err := git.CheckoutBranch(branch.Name); err != nil {
				s.Verbose(true).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), not able to rebase your local branch %q onto %q.", project.Name, branch.Name, branch.TrackingBranch),
					"Please do it manually.",
				})
			}
			rebaseSuccess, err := tryRebase(jirix, project, branch.TrackingBranch)
			if err != nil {
				return err
			}
			if rebaseSuccess {
				s.Verbose(showUpdateLogs).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), rebased your local branch %q on %q", project.Name, branch.Name, branch.TrackingBranch),
				})
			} else {
				s.Verbose(true).Output([]string{
					fmt.Sprintf("NOTE: For project (%v), not able to rebase your local branch %q onto %q.", project.Name, branch.Name, branch.TrackingBranch),
			...
			line1 := fmt.Sprintf("NOTE: For Project (%v), branch %v does not track any remote branch.", project.Name, branch.Name)
...
func computeOp(local, remote *Project, state *ProjectState, gc bool) operation {
	...
	case local != nil && remote != nil:
		...
		case state.CurrentBranch.TrackingBranch == "" && local.Revision != remote.Revision:
			return updateOperation{commonOperation{
				destination: remote.Path,
				project:     *remote,
				source:      local.Path,
