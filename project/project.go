	...
	if !git.IsOnBranch() {
		...
		if trackingBranch != "" {
			...
			if rebaseSuccess {
				...
				s.Verbose(showUpdateLogs).Output([]string{line})
			} else {
				line1 := fmt.Sprintf("NOTE: For project (%v), not able to rebase your local branch onto %v.", project.Name, trackingBranch)
				line2 := "Please do it manually."
				s.Verbose(true).Output([]string{line1, line2})
			}
			return nil
		} else {
