	...
	for _, branch := range branches {
		if branch.Tracking != nil { // tracked branch
			...
			if rebaseUntracked {
				...
				if err := scm.CheckoutBranch(branch.Name); err != nil {
					msg := fmt.Sprintf("For project %q, not able to rebase your untracked branch %q.", project.Name, branch.Name)
					msg += fmt.Sprintf("\nTo rebase it manually run 'git -C %q rebase %s'\n\n", relativePath, revision)
					jirix.Logger.Errorf(msg)
					continue
				}
				...
				if rebaseSuccess {
					...
					msg := fmt.Sprintf("For project %q, not able to rebase your untracked branch %q.", project.Name, branch.Name)
					msg += fmt.Sprintf("\nTo rebase it manually run 'git -C %q checkout %s; git -C %q rebase %s'\n\n", relativePath, branch.Name, relativePath, revision)
					jirix.Logger.Errorf(msg)
					continue
				}
