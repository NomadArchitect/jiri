...
func resolvePackageLocks(jirix *jiri.X, pkgs Packages) (PackageLocks, error) {
	...
	pkgs, _, err := removePkgsWithoutAccess(jirix, pkgs)
	if err != nil {
		return nil, err
	}
...
func FetchPackages(jirix *jiri.X, pkgs Packages, fetchTimeout uint) error {
	...
	pkgsWAccess, hasInternalPkgs, err := removePkgsWithoutAccess(jirix, pkgs)
	if err != nil {
		return err
	}
	...
	// Write flags to GNI File
	writeGNI := func(file, flag string, b bool) error {
		const disclaimer = "# This file is generated by jiri"
		if file == "" || flag == "" {
			return nil
		}
	...
	if hasInternalPkgs {
		access := len(pkgs) == len(pkgsWAccess)
		if err := writeGNI(path.Join(jirix.RootMetaDir(), jirix.PrebuiltGNIName), jirix.PrebuiltGNIFlag, access); err != nil {
			return err
		}
		// Write GNI file for zircon and garnet to allow smooth transition from hooks to packages
		// TODO: Remove them after the transition
		// Test existence of zircon/prebuilt
		if _, err := os.Stat(path.Join(jirix.Root, "zircon/prebuilt")); err == nil {
			if err := writeGNI(path.Join(jirix.Root, "zircon/prebuilt/config.gni"), "have_firmware", access); err != nil {
				jirix.Logger.Warningf("Failed to create \"zircon/prebuilt/config.gni\" due to error: %v", err)
			}
		}
		// Test existence of garnet/downloads
		if _, err := os.Stat(path.Join(jirix.Root, "garnet/tools")); err == nil {
			if err := writeGNI(path.Join(jirix.Root, "garnet/tools/cipd.gni"), "internal_access", access); err != nil {
				jirix.Logger.Warningf("Failed to create \"zircon/prebuilt/config.gni\" due to error: %v", err)
			}
		}
	}
...
func removePkgsWithoutAccess(jirix *jiri.X, pkgs Packages) (Packages, bool, error) {
