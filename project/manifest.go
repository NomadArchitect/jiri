...
func FetchPackages(jirix *jiri.X, pkgs Packages, fetchTimeout uint) error {
	...
	if _, err := ensureFile.Seek(0, 0); err != nil {
		return err
	}
	data, _ := ioutil.ReadAll(ensureFile)
	jirix.Logger.Debugf("Generated ensure file content:\n%v", string(data))
	if err := cipd.Ensure(jirix, ensureFilePath, jirix.Root, fetchTimeout); err != nil {
		return err
	}

	// Print a reminder to user to do cipd login if cipd does not have login info and
	// some packages are skipped due to lack of access
	if hasSkippedPkgs {
		cipdLoggedIn, err := cipd.CheckCipdLoggedIn(jirix)
		if err != nil {
			return err
		}
		if !cipdLoggedIn {
