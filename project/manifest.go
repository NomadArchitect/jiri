...
func (fetcher *commitMsgFetcher) fetch(jirix *jiri.X, gerritHost, path string) ([]byte, error) {
	bytes, ok := (*fetcher)[gerritHost]
	if !ok {
...
func applyGitHooks(jirix *jiri.X, ops []operation) error {
	...
	defer jirix.TimerPop()
	commitMsgFetcher := make(commitMsgFetcher)
	for _, op := range ops {
