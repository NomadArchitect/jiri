...
// pkgs using cipd.
func resolvePackageLocks(jirix *jiri.X, pkgs Packages) ([]PackageLock, error) {
	...
	// TODO: Remove this boilplate once we have a better package
	// layout that doesn't cause import cycles
	pkgLocks := make([]PackageLock, len(pkgInstances))
	for i, val := range pkgInstances {
		pkgLocks[i].PackageName = val.PackageName
		pkgLocks[i].InstanceID = val.InstanceID
	}
...
func resolveProjectLocks(jirix *jiri.X, projects Projects) ([]ProjectLock, error) {
	projectLocks := make([]ProjectLock, len(projects))
	i := 0
	for _, v := range projects {
		projectLocks[i].Remote = v.Remote
		projectLocks[i].Revision = v.Revision
		i++
	}
