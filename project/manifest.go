...
func FetchPackages(jirix *jiri.X, pkgs Packages, fetchTimeout uint) error {
	...
	if hasInternalPkgs {
		...
		if err := writeGNI(filepath.Join(jirix.RootMetaDir(), jirix.PrebuiltGNIFile), jirix.PrebuiltGNIFlag, access); err != nil {
			return err
		}
		...
		if _, err := os.Stat(filepath.Join(jirix.Root, "zircon/prebuilt")); err == nil {
			if err := writeGNI(filepath.Join(jirix.Root, "zircon/prebuilt/config.gni"), "have_firmware", access); err != nil {
				jirix.Logger.Warningf("Failed to create \"zircon/prebuilt/config.gni\" due to error: %v", err)
			}
		}
		// Test existence of garnet/downloads
		if _, err := os.Stat(filepath.Join(jirix.Root, "garnet/tools")); err == nil {
			if err := writeGNI(filepath.Join(jirix.Root, "garnet/tools/cipd.gni"), "internal_access", access); err != nil {
				jirix.Logger.Warningf("Failed to create \"zircon/prebuilt/config.gni\" due to error: %v", err)
			}
		}
	}
