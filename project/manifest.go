	...
	for _, v := range ld.Projects {
		if projectLock, ok := ld.ProjectLocks[ProjectLockKey(v.Key())]; ok {
			if v.Revision == "" {
				...
				// Returned error only contains 1 conflict. Therefore, write all conflicts to debug logger.
				jirix.Logger.Debugf("Project %+v has conflicting revisions in manifest and jiri.lock: %s:%s", v, v.Revision, projectLock.Revision)
				err = fmt.Errorf("Project %+v has conflicting revisions in manifest and jiri.lock: %s:%s", v, v.Revision, projectLock.Revision)
			}
		} else {
			jirix.Logger.Debugf("Project %q does not have matched lock entry in jiri.lock", v.Remote)
		}
	}
