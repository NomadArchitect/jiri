...
import (
	...
	"fuchsia.googlesource.com/jiri/cipdutil"
...
type Package struct {
	...
	Internal string   `xml:"internal,attr,omitempty"`
...
func (pack Package) Key() PackageKey {
	return PackageKey(pack.Name)
}
...
// RunPackages fetch prebuilt packages using cipd
func RunPackages(jirix *jiri.X, packs Packages, runPackagesTimeout uint) error {
	jirix.TimerPush("fetch cipd packages")
	defer jirix.TimerPop()

	tmpDir, err := ioutil.TempDir("", "run-cipd")
	if err != nil {
		return fmt.Errorf("not able to create tmp dir: %v", err)
	}
	//defer os.RemoveAll(tmpDir)

	ensureFile, err := ioutil.TempFile(tmpDir, "jiri*.ensure")
	if err != nil {
	...
	//versionFilePath = ensureFilePath[:-len(".ensure")] + ".version"
	...
	for _, pack := range packs {
		cipdDecl, err := pack.packageToCipdDeclaration()
		if err != nil {
			return err
		}
	...
	if err := cipdutil.Ensure(jirix, ensureFilePath, jirix.Root, runPackagesTimeout); err != nil {
		return err
	}
...
func (pack *Package) packageToCipdDeclaration() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error while parsing package path %s", subdir)
	}
