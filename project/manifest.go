...
func newGerritCommitMsgFetcher() func(*jiri.X, string, string) ([]byte, error) {
	commitHookMap := make(map[string][]byte)
	return func(jirix *jiri.X, gerritHost, path string) ([]byte, error) {
		bytes, ok := commitHookMap[gerritHost]
		if !ok {
			data, err := gerrit.FetchFileFromGerritHost(gerritHost, "/tools/hooks/commit-msg")
			if err != nil {
				...
				data, err = gerrit.FetchFileFromGerritHostSSO(jirix, gerritHost, "/tools/hooks/commit-msg")
				if err != nil {
					return nil, err
				}
			}
			commitHookMap[gerritHost] = data
			return data, nil
		}
...
func applyGitHooks(jirix *jiri.X, ops []operation) error {
	...
	defer jirix.TimerPop()
	commitMsgFetcher := newGerritCommitMsgFetcher()
	for _, op := range ops {
		if op.Kind() != "delete" && !op.Project().LocalConfig.Ignore && !op.Project().LocalConfig.NoUpdate {
			if op.Project().GerritHost != "" {
				...
				bytes, err := commitMsgFetcher(jirix, op.Project().GerritHost, "/tools/hooks/commit-msg")
				...
				if err != nil && err != gerrit.ErrSSOHelperNotExist {
					commitHook.Close()
					os.Remove(hookPath)
					return err
				}
