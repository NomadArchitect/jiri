...
func (ld *loader) warnOverrides(jirix *jiri.X) {
	if len(ld.ProjectOverrides) != 0 {
		for _, v := range ld.ProjectOverrides {
			...
			jirix.Logger.Warningf("Project %s(remote: %s) is overrided to revision %s, if this is not your intention, please run \"jiri override --delete %s %s\" to disable it.", v.Name, v.Remote, revision, v.Name, v.Remote)
	...
	if len(ld.ImportOverrides) != 0 {
		for _, v := range ld.ImportOverrides {
			...
			jirix.Logger.Warningf("Import %s(remote: %s) is overrided to revision %s, if this is not your intention, please run \"jiri override --import-manifest=%s --delete %s %s\" to disable it.", v.Name, v.Remote, revision, v.Manifest, v.Name, v.Remote)
			jirix.OverrideWarned = true
		}
	}
}
