	...
	for _, branch := range branches {
		...
		if v, ok := m[gitutil.LocalType+"/"+branch]; ok {
			...
			if string(v.TrackingBranch) != "" {
				b.TrackingBranch = &ReferenceState{
					Name: string(v.TrackingBranch),
				}
				if v, ok := m[gitutil.RemoteType+"/"+b.TrackingBranch.Name]; ok {
					b.TrackingBranch.Revision = string(v.Revision)
				}
			}
		}
