...
func fetchFileWithJar(jirix *jiri.X, gerritHost, path string, jar http.CookieJar) ([]byte, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		if jirix != nil {
			jirix.Logger.Debugf(httpHeadersToText(resp))
		}
...
func httpHeadersToText(resp *http.Response) string {
	...
	if header != nil {
		output.WriteString("HTTP Respond Header: \n")
		for k, v := range header {
			output.WriteString(fmt.Sprintf("%s: %v\n", k, v))
		}
	}
...
func FetchFileSSO(jirix *jiri.X, gerritHost, path string) ([]byte, error) {
	...
	if err == ErrHTTPForbidden {
		return nil, fmt.Errorf("git cookie for %s is invalid, please follow the instructions at %q", gerritHost, fmt.Sprintf(generatePasswordURL, hostName))
	}
